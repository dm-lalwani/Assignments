// Gemini Api
let heading = "";
let jobSuggestionsDisplayContainer = document.getElementById(
  "jobSuggestionsDisplayContainer"
);
...
async function Search(prompt, heading) {
  ...
  try {
    ...
    jobSuggestionsDisplayContainer.innerHTML =
      "<p>Error fetching job suggestions. Please try again later.</p>";
    jobSuggestionsDisplayContainer.style.display = "block";
  } finally {
...
document.getElementById("jobSuggestionButton").addEventListener("click", () => {
  prompt = "";
  let skills_dsp = document.querySelectorAll(
    ".skills-items .preview-item .preview-item-val"
  );
...
document
  .getElementById("roadmapSuggestionButton")
